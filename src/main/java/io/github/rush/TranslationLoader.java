...
public final class TranslationLoader {
    ...
    public static void register(Main plugin) {
        ...
        try (InputStream in = plugin.getClass().getClassLoader().getResourceAsStream("translations/fr_fr.json")) {
            ...
            final TranslationRegistry registry = TranslationRegistry.create(Key.key("rush", "translations"));

            registry.defaultLocale(Locale.FRANCE);

            for (Map.Entry<String, String> entry : loaded.entrySet()) {
                registry.register(entry.getKey(), Locale.FRANCE, new MessageFormat(entry.getValue(), Locale.FRANCE));
            }
